# ================= COINDCX + BINANCE VISION - FINAL PRODUCTION BOT =================
...
# TIME: 3-Day Deadline Build | Ready to Deploy
...
# All 100 confirmed on CoinDCX Futures + Binance API as of May 2026
COINS = [
...
# ================= 15 SHADOW PATTERNS - TRACK ONLY, NO SIGNALS =================
SHADOW_PATTERNS = [
...
def market_session_allows_trade():
    ...
    if 0 <= hour < 6: return False # 12am-6am IST dead zone
    if hour == 19 and datetime.now(IST).minute < 30: return False # US open volatility
    return True

# ================= PATTERN DETECTION - 10 PRIMARY =================
def detect_primary_patterns(closes, highs, lows, opens, volumes, price, trend_score, rsi_val, momentum, vol_strength, atr_val):
    ...
    if abs(trend_score) >= 3:
        patterns.append("EMA Trend")

    # 2. Breakout
    resistance = max(highs[-20:])
    if price > resistance * 1.001 and vol_strength >= 130 and closes[-1] > opens[-1]:
        patterns.append("Breakout")

    # 3. Pullback to 20 EMA
    ema20 = ema(closes, 20)
    if abs(trend_score) >= 2 and abs(price - ema20)/ema20 < 0.008 and ((trend_score > 0 and closes[-1] > opens[-1]) or (trend_score < 0 and closes[-1] < opens[-1])):
        patterns.append("Pullback to 20 EMA")

    # 4. RSI Reversal
    if (rsi_val < 30 and closes[-1] > opens[-1]) or (rsi_val > 70 and closes[-1] < opens[-1]):
        patterns.append("RSI Reversal")

    # 5. Momentum Surge
    if abs(momentum) >= 3 and abs(closes[-1] - closes[-5])/closes[-5] * 100 >= 2:
        patterns.append("Momentum Surge")

    # 6. Volume Spike
    if vol_strength >= 200 and ((closes[-1] > opens[-1] and trend_score >= 0) or (closes[-1] < opens[-1] and trend_score < 0)):
    ...
    if len(lows) >= 20:
        ...
        if abs(recent_low - prev_low)/prev_low < 0.005 and rsi_val < 40 and closes[-1] > opens[-1]:
            patterns.append("Double Bottom")

    # 8. Bull Flag
    if momentum > 5 and max(closes[-5:]) - min(closes[-5:]) < atr_val * 1.5 and closes[-1] > closes[-2]:
        patterns.append("Bull Flag")

    # 9. Trend Continuation
    if closes[-1] > closes[-5] > closes[-10] and trend_score >= 2 and vol_strength >= 110:
        patterns.append("Trend Continuation")

    # 10. Range Break + Retest
    if len(highs) >= 25 and price > max(highs[-20:]) and min(lows[-5:]) > max(highs[-25:-20]) * 0.998:
        patterns.append("Range Break + Retest")

    return patterns

def detect_shadow_patterns(closes, highs, lows, opens, volumes, price, trend_score, rsi_val):
    # Simplified detection for tracking only
    patterns = []
...
def get_smart_sl_tp(closes, highs, lows, direction, entry, atr_val, momentum, vol_strength):
    # Smart SL at swing point
    if direction == "BUY":
        swing_low = min(lows[-10:])
        sl = swing_low - atr_val * 0.5
        sl = max(sl, entry * 0.98) # Max 2% risk
    else:
    ...
    if atr_val / entry < 0.01: tp_mult = 1.2 # Dead market
...
def generate_signal(coin):
    ...
    # Time filter
    if not market_session_allows_trade():
        return None, None
    ...
    for tf in ["5m", "15m", "30m"]:
        ...
        trend_score = sum([1 if price > ema(get_candles(symbol,t,50)[0],20) else -1 for t in ["5m","15m","30m","1h"] if get_candles(symbol,t,50)[0]])
        ...
        pattern = primary_patterns[0] # Take first match
    ...
    # Detect shadow patterns for tracking
    shadow_patterns = []
    if best_signal:
...
def send_pattern_report():
    ...
    for p in PRIMARY_PATTERNS:
        ...
        if s["signals"] > 0:
            ...
            msg += f"{p}: {s['signals']} sig | {wr}% WR | {avg_pnl}%\n"
    ...
    for p in SHADOW_PATTERNS:
        ...
        if s["signals"] >= 3:
            ...
            if wr >= 65 and s["signals"] >= 10:
                msg += f" ⚡ PROMOTE? {p} performing well!\n"
    ...
    while True:
        for coin, trade in list(active_trades.items()):
            ...
            if profit_distance >= sl_distance and not trade.get("trailed"):
                if trade["direction"] == "BUY":
                    trade["sl"] = trade["entry"] * 1.003 # Breakeven + 0.3%
                else:
                    trade["sl"] = trade["entry"] * 0.997
                trade["trailed"] = True
                send_telegram(f"🔒 <b>TRAIL ACTIVATED {coin}</b>\nSL moved to breakeven. Risk-free now!")
            ...
            if closes and len(closes) > 20:
                ...
                if new_trend!= trade["direction"] and trade.get("last_trend_alert", 0) < time.time() - 1800:
                    send_telegram(f"⚠️ <b>TREND REVERSAL {coin}</b>\nYour {trade['direction']} trade now against trend.\nCurrent: {price:.4f} | PnL: {pnl:.2f}%\nConsider closing!")
                    trade["last_trend_alert"] = time.time()

            # Live update every 30 min
            if trade.get("last_update", 0) < time.time() - 1800:
                send_telegram(f"📈 <b>LIVE UPDATE {coin}</b>\nDir: {trade['direction']} | Pattern: {trade['pattern']}\nEntry: {trade['entry']:.4f}\nNow: {price:.4f}\nPnL: {pnl:.2f}%\nTP: {trade['tp']:.4f} | SL: {trade['sl']:.4f}")
    ...
    try:
        for coin in list(active_trades.keys()):
            ...
            if data.get("results"):
                ...
                send_telegram(f"📰 <b>NEWS ALERT {coin}</b>\n{title}")
    except: pass

# ================= MAIN LOOP =================
def main():
    load_trade_history()
    send_telegram("🚀 <b>BOT ONLINE - 24/7 MODE</b>\n100 Coins | 10 Primary + 15 Shadow Patterns\nActive Tracking | Reversal Alerts | News Feed\nReady for signals!")
    ...
    while True:
        try:
            ...
            for coin in COINS:
                ...
                # Track shadow patterns even if no primary signal
